...
only after one bounded, idempotent start of the exact installed
`openclaw-gateway.service` unit. Service timeout or nonzero exit fails closed
with sanitized bounded diagnostics. Use one two-minute absolute reconnect
deadline and a 12-attempt cap. Retry immediate connection-refused, error, and
timeout outcomes, but fail closed on unexpected pairing. Limit each
connect/status attempt to the smaller of 15 seconds and the remaining deadline;
cancellation must dispose the candidate to interrupt a stalled WebSocket
handshake. Restart the wizard and replay prior answers only after a confirmed
connection. Preserve the existing two-restart cap.
