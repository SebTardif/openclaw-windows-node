...
| chat-provider-thinking-level-clear-closed | closed | src/OpenClaw.Tray.WinUI/Chat/OpenClawChatDataProvider.cs | private thinking-level clear state dictionaries, operation records, retry counters, timers, and connection-generation transitions | ThinkingLevelClearReconciler | narrow calls that begin operations, apply effective thinking levels, and execute emitted refresh requests | OpenClawChatDataProvider never regains an independent reasoning-clear state machine | ThinkingLevelClearReconcilerArchitectureTests.Provider_DoesNotOwnThinkingLevelClearReconciliationState | source-shape | when OpenClawChatDataProvider is replaced by narrower chat coordinators |
