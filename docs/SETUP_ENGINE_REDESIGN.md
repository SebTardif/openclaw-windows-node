...
The bundled `default-config.json` ships with the tray executable and provides secure defaults (loopback bind, pinned gateway version, WSL isolation). The UI recommends WSL while offering native Windows as a lower-isolation alternative; headless callers retain the explicit `InstallMode` config default. Values can be overridden via config file or environment variables.
