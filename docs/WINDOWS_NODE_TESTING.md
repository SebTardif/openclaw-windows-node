...
connection and invokes one bounded, idempotent start of the exact installed
`openclaw-gateway.service` unit. Service timeout or nonzero exit fails closed
with sanitized bounded diagnostics. It then retries fresh operator clients
under one two-minute absolute deadline and a 12-attempt cap. Each connect/status
attempt uses the smaller of 15 seconds and the remaining budget; cancellation
disposes the candidate so a stalled WebSocket handshake cannot defeat the
deadline. Connection-refused, error, and timeout outcomes are retryable.
Unexpected pairing fails closed. Wizard start and answer replay occur only
after a confirmed connection, with at most two restart/replay cycles.
