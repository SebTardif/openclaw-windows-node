        ...
        if (_nodeConnector is INodeConnectorReconnectPolicy reconnectPolicy)
        {
                ...
                CancellationToken authorizationCancellationToken)
            {
                ...
                var reconnectAuthorization = await _endpointSecurity.AuthorizeCredentialAsync(
                    target.Record,
                    credential,
                    authorizationCancellationToken).ConfigureAwait(false);
                if (reconnectAuthorization.Allowed &&
                    expectedEndpointOwnership is not null &&
                    reconnectAuthorization.OwnershipProof != expectedEndpointOwnership)
                {
                    reconnectAuthorization = new EndpointCredentialAuthorization(
                        false,
